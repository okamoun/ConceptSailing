...
export default function ManageClient({ action }: Props) {
  ...
  useEffect(() => {
    ...
    if (!user) { router.replace('/login?redirect=/reviews/manage'); return; }
    if (userProfile?.role !== 'client') { setError('Access denied.'); setLoading(false); return; }

    const token = userProfile.reviewToken;
    if (!token) { setError('No review is linked to your account.'); setLoading(false); return; }

    getReviewByToken(token)
      .then(r => {
        if (!r) { setError('Review not found. It may have already been processed.'); return; }
        setReview(r);
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4 py-16" style={bg}>
      <div className="w-full max-w-lg bg-white/15 backdrop-blur-sm border border-white/25 rounded-2xl p-8 space-y-5">
        ...
        {!authLoading && !loading && !error && !done && review && (
          <>
            {action === 'confirm' && (
              <div className="space-y-4">
                ...
                <Link href="/reviews/manage?action=edit" className="block text-center text-blue-200 hover:text-white text-xs underline">
                  Edit first
                </Link>
                <Link href="/reviews/manage?action=delete" className="block text-center text-red-300 hover:text-red-200 text-xs underline">
                  Delete this review
                </Link>
              </div>
            )}
            ...
            {action === 'edit' && (
              <form onSubmit={handleEdit} className="space-y-4">
                ...
                <p className="text-blue-200 text-xs">After saving, you will need to re-confirm via this page.</p>
                <button
                  type="submit"
                  disabled={saving}
                  className="btn-primary w-full py-3 text-sm disabled:opacity-60"
            ...
            {action === 'delete' && (
              <div className="space-y-4">
                ...
                <Link href="/reviews/manage?action=confirm" className="block text-center text-blue-200 hover:text-white text-xs underline">
                  ← Keep it
                </Link>
              </div>
            )}
