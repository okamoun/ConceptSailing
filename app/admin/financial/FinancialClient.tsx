...
function CharterRow({ cf }: { cf: CharterFinancials }) {
  ...
  return (
    <tr className="border-b border-white/10 hover:bg-white/5 transition-colors">
      ...
      <td className="px-3 py-2 text-blue-200 text-center">{nights}</td>
...
const TABLE_HEADERS = ['Client', 'Dates', 'Nights', 'Season', 'Charter Fee', 'APA', 'VAT', 'Relocation', 'Total'];
...
export default function FinancialClient() {
  ...
  // Season utilization: High = Jul(6)+Aug(7), Mid = Jun(5)+Sep(8), Low = rest
  // Weeks available approximation: High ≈ 8w, Mid ≈ 8w, Low ≈ 36w
  const SEASON_CAP: Record<string, number> = { high: 8, mid: 8, low: 36 };

  const seasonNights: Record<string, number> = { high: 0, mid: 0, low: 0 };
  if (summary) {
    for (const cf of summary.confirmed) {
      seasonNights[cf.tier] += cf.nights;
    }
  }
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-blue-200 text-xs mt-0.5">Confirmed &amp; signed charters only · season assignment by start date</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setYear(y => y - 1)}
              className="text-blue-300 hover:text-white px-3 py-1.5 rounded-lg hover:bg-white/10 transition-colors text-sm"
            >
              ‹
            </button>
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <StatCard
                ...
                value={fmt(summary.totalCharterFee)}
                sub="charter fees ex-VAT"
              />
              <StatCard
                label="APA Pool"
                value={fmt(summary.totalApa)}
                sub={`${pricing.apaPercent}% of charter fee`}
              />
              <StatCard
                label="VAT Collected"
                value={fmt(summary.totalVat)}
                sub={`${pricing.vatPercent}%`}
              />
              <StatCard
            ...
            <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl px-4 py-4">
              <h2 className="text-white font-semibold text-sm mb-4">Monthly Charter Revenue</h2>
            ...
            <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl px-4 py-4">
              ...
              <div className="space-y-3">
                {(['high','mid','low'] as const).map(t => {
                  ...
                  const bookedRev = summary.confirmed
                    .filter(cf => cf.tier === t)
                    .reduce((s, cf) => s + cf.charterFee, 0);
                  return (
                    <div key={t}>
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-blue-200 text-xs">
                          {bookedWeeks.toFixed(1)}w / {capWeeks}w available · {fmt(bookedRev)}
                        </span>
                      </div>
                      <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all ${SEASON_COLORS[t]}`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tfoot>
                        <tr className="border-t border-white/20 bg-white/5">
                          ...
                          <td className="px-3 py-2.5 text-white font-semibold text-right">{fmt(summary.totalApa)}</td>
                          ...
                          <td className="px-3 py-2.5 text-white font-semibold text-right">
                            {fmt(summary.confirmed.reduce((s, cf) => s + cf.relocation, 0))}
                          </td>
              ...
              {editPricing && (
                <div className="mt-3 bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-5 space-y-5">
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {(
                      [
                        { key: 'highSeasonRate', label: 'High Season Rate (€/week)',  note: 'Jul–Aug' },
                        { key: 'midSeasonRate',  label: 'Mid Season Rate (€/week)',   note: 'Jun, Sep' },
                        { key: 'lowSeasonRate',  label: 'Low Season Rate (€/week)',   note: 'other months' },
                        { key: 'apaPercent',     label: 'APA (%)',                    note: 'of charter fee' },
                        { key: 'vatPercent',     label: 'VAT (%)',                    note: 'of charter fee' },
                        { key: 'relocationFee',  label: 'Relocation Fee (€)',         note: 'flat, when off-base' },
                      ] as const
                    ).map(({ key, label, note }) => (
                      <div key={key}>
